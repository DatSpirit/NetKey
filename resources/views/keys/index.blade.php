@push('styles')
    <style>
        ...
        .nk-key-action-details:hover {
            background: rgba(37, 99, 235, 0.12);
        }

        .dark .nk-key-action-details {
        ...
        .nk-key-action-extend:hover {
            background: rgba(34, 197, 94, 0.15);
        }

        .dark .nk-key-action-extend {
            background: rgba(34, 197, 94, 0.08);
            color: #4ade80;
        }
        ...
        .nk-key-action-copy:hover {
            background: var(--bg, #f3f4f6);
            color: var(--fg, #111827);
        }

        .dark .nk-key-action-copy {
            background: #21262d;
            color: #8b949e;
        }
        ...
        <div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
                    ...
                    <div style="display:flex;align-items:center;gap:24px">
                        <div style="text-align:center">
                            ...
                            <div
                                style="font-size:0.7rem;color:rgba(255,255,255,0.55);text-transform:uppercase;letter-spacing:1px">
                                {{ __('Total Keys') }}
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="nk-key-stat">
                    ...
                    <div style="font-size:1.75rem;font-weight:800;color:#16a34a;margin-top:8px">{{ $stats['active'] }}
                    </div>
                    <div
                        style="font-size:0.8rem;color:var(--fg-muted,#6b7280);margin-top:4px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px">
                        {{ __('Active') }}
                    </div>
                </div>
                <div class="nk-key-stat">
                    <div class="nk-key-stat-stripe stripe-top-amber"></div>
                    <div style="font-size:1.75rem;font-weight:800;color:#d97706;margin-top:8px">
                        {{ $stats['expiring_soon'] }}
                    </div>
                    <div
                        style="font-size:0.8rem;color:var(--fg-muted,#6b7280);margin-top:4px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px">
                ...
                <div class="nk-key-stat">
                    ...
                    <div style="font-size:1.75rem;font-weight:800;color:#2563eb;margin-top:8px">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                    ...
                    <div class="key-card group relative">
                        ...
                        <div style="padding:56px 20px 20px">
                            ...
                            <div style="margin-bottom:16px">
                                <span
                                    style="font-size:0.7rem;font-weight:700;color:var(--fg-muted,#6b7280);text-transform:uppercase;letter-spacing:1px">{{ __('Key Code') }}</span>
                                <div class="nk-key-code-box"
                                    style="display:flex;align-items:center;justify-content:space-between;margin-top:6px;padding:10px 12px">
                                    <code class="nk-key-code-text"
                                        style="font-size:0.95rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $key->key_code }}</code>
                            ...
                            <div style="space:0;font-size:0.8rem;margin-bottom:16px">
                                <div
                                    style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border,#f3f4f6)">
                                    <span style="color:var(--fg-muted,#6b7280)">{{ __('Created At') }}</span>
                                    <span
                                        style="font-weight:600;color:var(--fg,#111827)">{{ $key->created_at->setTimezone('Asia/Ho_Chi_Minh')->format('d/m/Y H:i') }}</span>
                                </div>
                                <div
                                    style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border,#f3f4f6)">
                                    <span style="color:var(--fg-muted,#6b7280)">{{ __('Activated At') }}</span>
                                    <span
                                        style="font-weight:600;color:{{ $key->activated_at ? '#16a34a' : 'var(--fg-muted,#9ca3af)' }}">{{ $key->activated_at ? $key->activated_at->setTimezone('Asia/Ho_Chi_Minh')->format('d/m/Y H:i') : __('Not Activated') }}</span>
                                </div>
                                <div
                                    style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border,#f3f4f6)">
                                    <span style="color:var(--fg-muted,#6b7280)">{{ __('Total Duration') }}</span>
                                    <span
                                        style="font-weight:700;color:var(--fg,#111827)">{{ number_format($key->duration_minutes) }}
                                        {{ __('minutes') }}</span>
                                </div>
                                <div
                                    style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border,#f3f4f6)">
                                    ...
                                    <span
                                        style="font-weight:600;color:{{ $key->isExpired() ? '#dc2626' : '#16a34a' }}">{{ $key->expires_at ? $key->expires_at->format('d/m/Y H:i') : '∞ ' . __('Permanent') }}</span>
                                </div>
                                <div
                                    style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border,#f3f4f6)">
                            ...
                            <div style="display:flex;gap:8px;padding-top:12px;border-top:1px solid var(--border,#e5e7eb)">
                                <a href="{{ route('keys.keydetails', $key->id) }}"
                                    class="nk-key-action-details">{{ __('Details') }}</a>
                                @if ($key->product_id)
                                ...
                                @endif
                                <button onclick="copyToClipboard('{{ $key->key_code }}')" class="nk-key-action-copy">📋
                                    {{ __('Copy') }}</button>
                            </div>
                        </div>
                    </div>
                @empty
                    <div class="col-span-full py-16 text-center">
                        <div
                            style="width:72px;height:72px;background:var(--bg-elevated,#f3f4f6);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 16px">
                            <svg width="32" height="32" fill="none" stroke="#9ca3af" stroke-width="1.5" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
                            </svg>
                        </div>
                        <h3 style="font-size:1.1rem;font-weight:700;color:var(--fg,#111827);margin-bottom:8px">
                            {{ __('You have no keys') }}
                        </h3>
