    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('Transaction Details') }}</h2>
    ...
    <div class="space-y-6">
        ...
        <div class="
            ...
            <div class="flex flex-col md:flex-row items-center justify-between">
                ...
                <div class="text-center">
                    <p class="text-sm opacity-90 mb-2">{{ __('Transaction Amount') }}</p>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                @if($transaction->product)
                    <div
                        ...
                        <div class="flex items-start space-x-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
                            <div
                                class="w-20 h-20 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center flex-shrink-0">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-4 text-sm">
                                    <span
                                        class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-full font-semibold">
                ...
                <div
                    ...
                    <div class="space-y-4">
                        <div
                            ...
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Order Code') }}</span>
                            <span
                                class="text-sm font-bold text-gray-900 dark:text-white">#{{ $transaction->order_code }}</span>
                        </div>

                        <div
                            class="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-gray-700">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Transaction ID') }}</span>
                        ...
                        <div
                            ...
                            @php
                                $statusConfig = [
                                    'success' => ['bg' => 'bg-green-100 dark:bg-green-900/50', 'text' => 'text-green-800 dark:text-green-300', 'label' => __('Success')],
                                    'pending' => ['bg' => 'bg-yellow-100 dark:bg-yellow-900/50', 'text' => 'text-yellow-800 dark:text-yellow-300', 'label' => __('Pending')],
                                    'failed' => ['bg' => 'bg-gray-100 dark:bg-gray-700', 'text' => 'text-gray-800 dark:text-gray-300', 'label' => __('Failed')],
                                    'cancelled' => ['bg' => 'bg-red-100 dark:bg-red-900/50', 'text' => 'text-red-800 dark:text-red-300', 'label' => __('Cancelled')],
                                ];
                        ...
                        <div
                            class="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-gray-700">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Payment Method') }}</span>
                        ...
                        <div
                            class="flex justify-between items-center pb-3 border-b border-gray-200 dark:border-gray-700">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Created At') }}</span>
                            <span
                        ...
                        @if($transaction->description)
                            <div class="pt-2">
                                <p class="text-sm text-gray-600 dark:text-gray-400 mb-2">{{ __('Description') }}</p>
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">{{ __('Payment Summary') }}</h3>

                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Subtotal') }}</span>
                            <span
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Tax') }} (0%)</span>
                            <span class="text-sm font-semibold text-gray-900 dark:text-white">0 VND</span>
                        </div>

                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600 dark:text-gray-400">{{ __('Discount') }}</span>
                            <span class="text-sm font-semibold text-green-600 dark:text-green-400">- 0 VND</span>
                        </div>

                        <div
                            class="pt-3 border-t-2 border-gray-200 dark:border-gray-700 flex justify-between items-center">
                            <span class="text-lg font-bold text-gray-900 dark:text-white">{{ __('Total') }}</span>
                            <span
                                class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">{{ number_format($transaction->amount) }}
                                VND</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center space-x-3">
                            <div
                                ...
                                <span
                                    class="text-white font-bold">{{ strtoupper(substr($transaction->user->name, 0, 2)) }}</span>
                            </div>
                            <div>
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">{{ __('Quick Actions') }}</h3>

                    <div class="space-y-2">
                        @if($transaction->status === 'pending')
                            <button
                                class="w-full px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-medium transition-all flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            ...
                            <button
                                class="w-full px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-medium transition-all flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @elseif($transaction->status === 'success')
                            <button onclick="printInvoice()"
                                class="w-full px-4 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-medium transition-all flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <button
                                class="w-full px-4 py-3 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl font-medium hover:bg-gray-300 dark:hover:bg-gray-600 transition-all flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
