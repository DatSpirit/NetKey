<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <div class="p-2 bg-purple-700 rounded-xl shadow-lg">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
            </div>
            <h2 class="font-black text-2xl text-gray-800 dark:text-white tracking-tight">🔍 Admin Audit Log</h2>
        </div>
    </x-slot>

    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <p class="text-gray-500 dark:text-gray-400 text-sm mb-6">Lịch sử mọi hành động của Quản trị viên trên hệ thống</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 mb-4 shadow-sm">
            <form method="GET" class="flex gap-3 flex-wrap">
                ...
                <select name="action" class="bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg px-4 py-2 border border-gray-300 dark:border-gray-600 focus:outline-none">
                ...
                @if(request()->hasAny(['search','action','date']))
                    <a href="{{ route('admin.audit-log') }}" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition">Reset</a>
                @endif
            </form>
        </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-900 text-white">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">Admin</th>
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">Hành động</th>
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">Đối tượng</th>
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">Mô tả</th>
