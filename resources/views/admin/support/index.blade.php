<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-black text-2xl text-gray-800 dark:text-white tracking-tight">{{ __('Support Tickets') }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm">
                ...
                <div class="text-gray-500 text-sm">{{ __('Total Tickets') }}</div>
            </div>
            <div
            ...
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-200 dark:border-blue-700/50">
                ...
                <div class="text-gray-500 text-sm">{{ __('In Progress') }}</div>
            </div>
            <div
                class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-700/50">
                <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $stats['closed'] }}</div>
                <div class="text-gray-500 text-sm">{{ __('Closed') }}</div>
            </div>
        </div>
        ...
        <div
            ...
            <form method="GET" class="flex gap-3 flex-wrap">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="{{ __('Search by email, name, subject...') }}"
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-900 text-white">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">{{ __('Sender') }}</th>
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">{{ __('Subject') }}</th>
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">{{ __('Status') }}</th>
                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest opacity-70">{{ __('Time') }}</th>
                        <th class="px-4 py-3 text-right text-xs font-black uppercase tracking-widest opacity-70">{{ __('Actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    @forelse ($tickets as $ticket)
                        <tr class="hover:bg-blue-50/30 dark:hover:bg-gray-700/40 transition">
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-gray-200 max-w-xs truncate">{{ $ticket->subject }}
                            </td>
                            <td class="px-4 py-3">
                                @php
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-xs border {{ $badge }}">{{ $label }}</span>
                            </td>
                            <td class="px-4 py-3 text-gray-400 dark:text-gray-500 text-xs">
                                {{ $ticket->created_at->format('H:i d/m/Y') }}</td>
