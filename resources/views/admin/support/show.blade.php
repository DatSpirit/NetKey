<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-black text-2xl text-gray-800 dark:text-white tracking-tight">{{ __('Ticket') }} #{{ $ticket->id }}</h2>
        </div>
    </x-slot>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8 max-w-3xl">
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm">
            ...
            <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                ...
                <form method="POST" action="{{ route('admin.support.update-status', $ticket) }}" class="flex items-center gap-2">
                    ...
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="open" @selected($ticket->status === 'open')>{{ __('Open') }}</option>
                        <option value="in_progress" @selected($ticket->status === 'in_progress')>{{ __('In Progress') }}</option>
                        <option value="closed" @selected($ticket->status === 'closed')>{{ __('Closed') }}</option>
                    </select>
                </form>
            </div>
            ...
            <div class="px-6 py-5 border-b border-gray-200 dark:border-gray-700 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <p class="text-gray-400 text-xs uppercase font-semibold mb-1">{{ __('Sender') }}</p>
                ...
                <div>
                    <p class="text-gray-400 text-xs uppercase font-semibold mb-1">{{ __('Account') }}</p>
                    @if($ticket->user)
                        <p class="text-gray-900 dark:text-white">{{ __('User') }} #{{ $ticket->user->id }} — {{ $ticket->user->name }}</p>
                    @else
                        <p class="text-gray-400 italic">{{ __('Guest') }}</p>
                    @endif
                </div>
                ...
                @endif
                <div>
                    <p class="text-gray-400 text-xs uppercase font-semibold mb-1">{{ __('Status') }}</p>
                    @php
            ...
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <p class="text-gray-400 text-xs uppercase font-semibold mb-1">{{ __('Subject') }}</p>
                <p class="text-gray-900 dark:text-white text-lg font-medium">{{ $ticket->subject }}</p>
            </div>

            {{-- Nội dung --}}
            <div class="px-6 py-5">
                <p class="text-gray-400 text-xs uppercase font-semibold mb-3">{{ __('Requested Content') }}</p>
